    ...
    <body>
                    ...
                    <div style='position: absolute; top:325px; left:350px; width:800px; height:35px'>
                        <ul class='nav nav-tabs'>
                            <li role='presentation'>
                                <a style='text-decoration: none' href='".$GLOBALS['FS_INTRANET_RESTAURANT']."controller/raterProfile-controller.php?page=rater-profile&userid=". $userID . "'>Restaurant reviews</a></li>
                            </li>
                            <li role='presentation' class='active'><a href='#'>Item reviews</a></li>
                        </ul>
                    </div>";
            } else {
        ...
        <div style="position: absolute; top:385px; left:350px; width:800px; height:80px">
            <ul class="list-group">
                <?php
                    $itemCount = 0;
                    for ($i = 0, $size = count($raterProfileItemReviews); $i < $size; $i++) {
                        echo "<li class='list-group-item'>
                                <h5>Date posted: ".$raterProfileItemReviews[$i]['datepub']." <span class='glyphicon glyphicon-time' aria-hidden='true'></span></h5>
                                <h5>Review for: ".$raterProfileItemReviews[$i]['mi_name']."</h5>
                                <p> Rating: ".$raterProfileItemReviews[$i]['rating']." </p>
                                <p>Comments: ".$raterProfileItemReviews[$i]['comment']."</p>
                            </li>";
                        $itemCount++;
                    } 
                    if ($itemCount === 0) {
